...
while True:
    ...
    # draw a vertical line to divide the frame into two halves
    (h, w) = frame.shape[:2]
    midX = w // 2
    cv2.line(frame, (midX, 0), (midX, h), (255, 0, 0), 2)
    ...
    if len(cnts) > 0:
        ...
        # only proceed if the radius meets a minimum size and if the circularity is close to 1 (indicating a circle)
        if radius > 10:
            ...
            # check if the ball is on the left or right side of the frame
            if center[0] < midX:
                cv2.putText(frame, "Gol!!", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            if center[0] > midX:
                cv2.putText(frame, "Tidak Gol!!", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
